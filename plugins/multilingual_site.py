...
class MultilingualContentProcessor:
    ...
    def process_articles(self, articles: List[Article]) -> Dict[str, List[Article]]:
        ...
        for article in articles:
            ...
            if self._is_translation_file(article):
                logger.debug(f"Skipping translation file: {getattr(article, 'source_path', 'unknown')}")
                continue
            
            # Process original content (assume English)
            original_lang = self._detect_content_language(article)
            logger.debug(f"Processing article '{getattr(article, 'title', 'unknown')}' in language '{original_lang}'")
            ...
            if original_lang == 'en':
                ...
                if hasattr(article, 'date') and article.date:
                    article.locale_date = self.date_localizer.localize_date(article.date, 'en')
    ...
    def _find_translations(self, content) -> Dict[str, str]:
        ...
        for lang in self.supported_langs:
            ...
            if translation_file.exists():
                ...
                logger.debug(f"No translation file found for {lang}: {translation_file}")
        
        logger.debug(f"Found {len(translations)} translations: {list(translations.keys())}")
        return translations
    ...
    def _create_translated_article_from_content(self, original_article: Article, translation_content: str, lang: str) -> Article:
        ...
        new_metadata['url'] = f"{lang}/{original_slug}"  # Remove both leading and trailing slash to prevent double slash
        ...
        if hasattr(translated_article, 'date') and translated_article.date:
            localized_date = self.date_localizer.localize_date(translated_article.date, lang)
            ...
            if body_lines[0].strip() == '```markdown':
                ...
                for i, line in enumerate(body_lines[1:], 1):
                    if line.strip() == '```':
                        ...
                        nested_metadata, nested_body = self._parse_nested_markdown(nested_content)
    ...
    def _process_markdown_content(self, content: str) -> str:
        ...
        try:
            ...
            logger.debug(f"Markdown processing successful, converted {len(content)} chars to {len(processed_content)} chars")
...
class MultilingualOutputGenerator:
    ...
    def generate_language_article(self, writer, article: Article, context: Dict, template_getter):
        ...
        if not save_as:
            logger.warning(f"No save_as metadata for article '{article.title}', skipping")
            return
    ...
    def _copy_images_for_translated_article(self, article: Article):
        ...
        try:
            ...
            if copied_images:
                logger.debug(f"Copied {len(copied_images)} images for translated article {article.title}")
    ...
    if hasattr(content_generator, 'articles'):
        for article in content_generator.articles:
            try:
                ...
                if hasattr(article, 'date') and article.date and not hasattr(article, 'locale_date'):
                    article.locale_date = date_localizer.localize_date(article.date, default_lang)
