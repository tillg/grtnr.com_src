...
def process_content(content):
    if hasattr(content, '_content') and not hasattr(content, '_toc_processed'):
        ...
        for ul in soup.find_all('ul'):
            ...
            links = ul.find_all('a', href=lambda href: href and href.startswith('#'))
            if links and len(links) > 0:
                # Add the toc class to this ul
                ul['class'] = ul.get('class', []) + ['toc']
                # Also fix nested ul elements to ensure proper indentation
                for nested_ul in ul.find_all('ul'):
                    nested_ul['class'] = nested_ul.get('class', []) + ['toc']
                modified = True
