...
from translation_utils import translate_document, get_translation_path, cleanup_old_translations
    ...
    def _filter_translatable_content(self) -> List[Any]:
        ...
        if test_limit and test_limit.isdigit():
            ...
            if len(all_content) > test_limit:
                ...
                logger.info(f"Applied test limit: processing only {test_limit} content items out of {len(all_content)} total")
        
        logger.info(f"Processing {len(all_content)} content items for translation")
        return all_content

    def _process_translations_simplified(self, content_list: List[Any]) -> Dict[str, int]:
        ...
        for target_lang in self.config.target_languages:
            try:
                ...
                if was_updated:
                    ...
                    stats['up_to_date'] += 1
                    logger.debug(f"Translation for {target_lang} was already up to date")
...
def translate_all_articles(article_generator):
    ...
    logger.info(f"Translation plugin running with {len(article_generator.articles)} articles")
    ...
    try:
        ...
        if translation_generator.config:
            ...
            logger.info(f"Using all {len(article_generator.articles)} articles for translation")
