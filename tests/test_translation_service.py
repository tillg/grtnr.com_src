...
class TestExtensionFileManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.article_dir = os.path.join(
            self.test_dir, "articles", "2025-01-01-test-article"
        )
    ...
    def test_is_translation_current(self):
        ...
        self.assertTrue(
            self.manager.is_translation_current(self.test_file, "de", file_hash)
        )

        # Should not be current with different hash
        self.assertFalse(
            self.manager.is_translation_current(self.test_file, "de", "different_hash")
        )
