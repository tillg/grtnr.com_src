...
def test_plugin_duplicate_issue():
    ...
    if both_exist := (
        automatic_translation_file.exists() and translation_service_file.exists()
    ):
        print("❌ Both files exist - this creates a conflict!")
        print(
            "💡 The plugin tries to import TranslationService, but there are TWO modules with that name:"
        )
