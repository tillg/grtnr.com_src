...
excerpt: L'application de contacts que je souhaite vraiment — un système unique pour toutes mes relations sur iPhone, Mac et iPad, avec des listes au lieu de comptes, une capture intelligente, et le réseautage intégré pour les conférences.
title: Peoplez
...
translate_date: 2026-02-27T10:30:14.106524+00:00
...
Tous les contacts vivent au même endroit. Les professionnels et les privés ne sont pas des comptes séparés — ce sont des **listes**. Par défaut, je vois tout le monde. Quand je veux me concentrer, je filtre par liste.
...
### Capture intelligente — réseautage de conférence intégré

Quand je suis à une conférence et que je rencontre quelqu'un, je le capture de trois manières :

**Photo de badge** — Les badges sont "quasi-contacts" mais pas des contacts. Une photo de badge est une entrée brute. Peoplez extrairait le nom et l'entreprise (OCR), créerait un brouillon de contact et conserverait la photo du badge comme provenance ("rencontré à ...").

**Lien LinkedIn** — Le moyen le plus rapide de "bien identifier" quelqu'un. [LinkedIn positionne les codes QR](https://www.linkedin.com/help/linkedin/answer/a525286/using-a-linkedin-qr-code-to-connect-with-members) comme un moyen de se connecter avec les personnes que vous rencontrez hors ligne. Peoplez récupérerait les connexions faites sur LinkedIn et enrichirait ou mettrait à jour le contact - en une action au lieu de trois. Peoplez rechercherait les liens LinkedIn récemment ajoutés et me suggérerait de les ajouter, avec la possibilité d'ajouter des données.

**Note rapide** — Parfois, vous ne voulez pas sortir votre téléphone en pleine conversation. Vous vous souvenez juste : _"Anna — travaille chez X — a parlé du déploiement iOS — suivi en mars."_ Peoplez transformerait cette note en contact (ou l'associerait à un existant) et ajouterait le contexte sous forme de données structurées, pas de texte aléatoire qui se perd.
...
Apple et Google ont tous deux un champ de notes sur les contacts — il est juste sous-utilisé parce que rien ne le remplit automatiquement. Peoplez en ferait la norme, pas l'exception. Peoplez est juste un portail / un tuyau organisé vers mes Contacts - les données du contact sont toujours stockées dans mon application Contacts d'Apple.

### Associer, dédupliquer, enrichir

Lorsqu'un nouveau contact arrive, Peoplez :

1. **Associe** — "Cela ressemble à Anna Müller déjà dans vos contacts — mettre à jour ?"
2. **Déduplique** — Fusionne les doublons à travers ce qui était autrefois des comptes séparés
3. **Enrichit** — Récupère l'entreprise, le titre, le lien de profil (lorsque permis), conserve la source
