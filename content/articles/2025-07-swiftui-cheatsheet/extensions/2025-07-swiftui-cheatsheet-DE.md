...
date: 2025-08-03
image: swiftui.png
excerpt: Mein Spickzettel, erstellt während des Kurses [100 Days of SwiftUI](https://www.hackingwithswift.com/100/swiftui/).
...
translate_date: 2026-02-27T10:15:21.827718+00:00
...
Aber es ist eine Menge Inhalt, daher hier meine Notizen - hoffentlich in einem leicht navigierbaren Spickzettelformat. Ich habe eine grobe Struktur im Kopf, werde den Inhalt aber nur dann ausfüllen, wenn ich ihn brauche. Erwarten Sie also keine vollständige Übersicht!

[TOC]

## Swift

Für einen umfassenden Überblick siehe [Lernen Sie das Wesentliche von Swift in einer Stunde](https://www.hackingwithswift.com/articles/242/learn-essential-swift-in-one-hour).
...
- Das Entpacken eines Optionals ist der Prozess, in eine Box zu schauen, um zu sehen, was sie enthält: Wenn ein Wert enthalten ist, wird er zur Verwendung zurückgegeben, andernfalls wird nil enthalten sein.
- Wir können `if let` verwenden, um Code auszuführen, wenn das Optional einen Wert hat, oder `guard let`, um Code auszuführen, wenn das Optional keinen Wert hat – aber bei `guard` müssen wir die Funktion danach immer verlassen.
...
- [Interactful](https://apps.apple.com/de/app/interactful/id1528095640?l=en-GB) ist ein nettes Tool, um die verschiedenen Ansichten und Komponenten zu navigieren und auszuprobieren.
...
Verwenden Sie sie, wenn viele Schaltflächen / Optionen verfügbar sind. Für Beispielcode schauen Sie sich [dieses Repository](https://github.com/tillg/100DaysOfSwiftUI/blob/main/16.5-AlertAndConfirmation/AlertAndConfirmation/AlertAndConfirmation/ContentView.swift) an.
...
Hinweis: Textfelder mit unterschiedlicher Formatierung können zusammengefügt werden, um ein großes Textfeld mit unterschiedlich formatierten Teilen zu bilden:
...
Und Sie erhalten einen Text mit kombinierten unterschiedlichen Formatierungen:
...
Schaltflächen in Listen: Wenn Sie eine Schaltfläche in einer Liste platzieren, wird das GESAMTE Listenelement anklickbar! Wenn es mehr als eine Schaltfläche in einer Liste gibt, wird, wo auch immer Sie auf das Listenelement klicken, ALLE Schaltflächen nacheinander geklickt!
...
### Werte an Ansichten übergeben & zurückgeben

Wie in [Auswählen und Bearbeiten von Kartenanmerkungen](https://www.hackingwithswift.com/books/ios-swiftui/selecting-and-editing-map-annotations) gesehen
...
Um Daten an den aufrufenden Code _zurückzugeben_, übergebe ich eine Methode `onSave`. Erstellen Sie zuerst eine zusätzliche Variable in meiner View-Struktur:

```swift
var onSave: (Location) -> Void
```

und erweitern Sie dann den Initialisierer wie folgt:
...
Dieser `@escaping`-Teil ist wichtig und bedeutet, dass die Funktion für die spätere Verwendung gespeichert wird, anstatt sofort aufgerufen zu werden, und er ist hier erforderlich, da die `onSave`-Funktion nur aufgerufen wird, wenn der Benutzer auf Speichern drückt.
...
4. Ein `didSet` für die Daten, sodass sie bei jeder Änderung oder Hinzufügung automatisch gespeichert werden.

Die Daten `Codable` zu machen, ist oft nicht allzu schwer: Solange die Komponenten `Codable` sind, ist es auch die gesamte Klasse.
