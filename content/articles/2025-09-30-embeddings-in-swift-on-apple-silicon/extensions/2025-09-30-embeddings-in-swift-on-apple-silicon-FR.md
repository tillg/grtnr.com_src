...
date: 2025-09-30
image: embedding.png
excerpt: Apple a introduit de nouveaux modèles d'intégration que je souhaite utiliser pour mesurer la proximité des textes. Voici mon parcours sur la façon de les créer et de les rendre efficaces.
...
translate_date: 2026-02-27T10:17:10.055211+00:00
generated_by: simplified-translation-system
---

**Travail en cours !!**

Je suis en train d'apprendre le langage de programmation Swift. Je le fais en suivant le formidable cours [100 Days of SwiftUI](https://www.hackingwithswift.com/100/swiftui/) de Paul Hudson. C'est un cours fantastique, qui couvre d'abord les bases de Swift puis SwiftUI.

Lorsque j'essaie d'appliquer mes apprentissages à de petits projets, j'ai toujours beaucoup de questions qui surgissent, et la source la plus fiable pour chercher des réponses est le cours, c'est-à-dire ses pages. Je dois donc rechercher sur le site de Paul Hudson, plus précisément dans les pages de son cours.
...
Dans l'ancien système d'intégration d'Apple qui se trouve dans le [cadre Natural Language](https://developer.apple.com/documentation/NaturalLanguage), les fonctions pour y parvenir sont facilement accessibles et très bien expliquées dans l'article [Trouver des similitudes entre des morceaux de texte](https://developer.apple.com/documentation/naturallanguage/finding-similarities-between-pieces-of-text).
...
- Est multilingue et interlinguistique : En s'entraînant sur plusieurs langues simultanément, le modèle aligne les espaces sémantiques entre les langues, donc “chien” et “dog” sont intégrés à proximité.
- Prend en charge plus de langues
- Fonctionne entièrement sur l'appareil : Le modèle respecte la vie privée de l'utilisateur et fonctionne hors ligne. Seuls de petits fichiers de modèle sont téléchargés lorsque nécessaire, et ceux-ci sont mis en cache à l'échelle du système.
...
- Une `Bundle Extension` qui lit les morceaux à partir du fichier JSON ([Code]()). Note : Cela est bien sûr inspiré du [cours de Paul Hudson](https://www.hackingwithswift.com/example-code/system/how-to-decode-json-from-your-app-bundle-the-easy-way) 😜
...
L'autre test consiste à calculer les distances entre des paires de phrases :
...
timerReport("Nom du minuteur") // Imprime mes statistiques de minuteur
```

Mon `timerTrack` renvoie également le résultat de son bloc et fonctionne en mode asynchrone. Nous pouvons donc faire des choses comme ceci :
...
Cela m'a pris un certain temps pour digérer cela, mais voici ce à quoi cela se résume :
...
Notez que dans ma base de code, j'ai enveloppé cela comme [extensions à `NLContextualEmbeddingResult`](https://github.com/tillg/SwiftEmbeddings/blob/main/SwiftEmbeddings/SwiftEmbeddings/NLContextualEmbeddingExtension.swift).
...
Wow ! Il faut 7x moins de temps pour calcul