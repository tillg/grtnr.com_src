...
translate_date: 2026-02-05T15:26:35.485285
generated_by: simplified-translation-system
---

**Remarque** : Le dépôt GitHub associé à cet article se trouve [ici](https://github.com/tillg/SwiftDataOniCloud).

Alors que j'apprenais Swift & SwiftUI et commençais à l'utiliser, j'ai souvent eu des difficultés à utiliser CloudKit. Trop souvent... Voici une configuration minimale que j'ai utilisée pour me rappeler comment j'ai mis en place les choses et documenter mes apprentissages.
...
- [Étape par étape](#étape-par-étape)
  ...
  - [Rendre le modèle compatible CloudKit](#rendre-le-modèle-compatible-cloudkit)
...
- [Erreurs fréquentes](#erreurs-fréquentes)
  ...
  - [Les droits ont été modifiés pendant la compilation](#les-droits-ont-été-modifiés-pendant-la-compilation)
  - [Impossible d'initialiser sans un compte iCloud (CKAccountStatusNoAccount).](#impossible-d-initialiser-sans-un-compte-icloud-ckaccountstatusnoaccount)

## Point de départ

Presque tout ce que je sais sur SwiftUI provient de l'excellent tutoriel [100 Days of SwiftUI](https://www.hackingwithswift.com/100/swiftui) de Paul Hudson. Il fait un travail fantastique en expliquant SwiftUI et offre ce cours bien entretenu gratuitement.

Pour ne pas avoir à tout recommencer à zéro, j'utilise la leçon du cours qui introduit SwiftData : Au [jour 53](https://www.hackingwithswift.com/100/swiftui/53), Paul commence un nouveau projet appelé _Bookworm_ qui utilise SwiftData pour stocker ses données de livres. À la fin du [jour 55](https://www.hackingwithswift.com/100/swiftui/55), il a construit une petite application mignonne qui stocke ses données dans SwiftData. Si vous n'êtes pas à l'aise avec SwiftData ou ne vous en souvenez pas, je vous recommande vivement de regarder les vidéos de ces 3 jours. Son application utilise SwiftData, mais pas dans le cloud, juste localement sur l'appareil.
...
Ensuite, en dessous, nous avons besoin d'un conteneur. Cliquez sur `+` et entrez un nom. Le nom habituel serait l'identifiant de bundle, dans mon cas c'est `com.grtnr.Bookworm` (je le copie et le colle simplement depuis le champ _Bundle Identifier_).
...
Pour garder les appareils synchronisés, iCloud utilise la _Notification Push à Distance_, nous devons donc ajouter ce service également. Cliquez à nouveau sur `+ Capability` et recherchez _Back..._ :
...
**Remarque** : Probablement maintenant, le nom du conteneur est passé de rouge à noir car il a été créé dans iCloud.

### Rendre le modèle compatible CloudKit

Lorsque nous lançons l'application maintenant, nous obtenons une erreur similaire à celle-ci :
...
La raison est qu'un modèle doit respecter certaines restrictions. Lors de la création de modèles pour les utiliser avec CloudKit, nous devons respecter certaines règles :
...
Lorsqu'on enquête sur ce qui se passe, il y a un outil très utile : La [Console CloudKit](https://icloud.developer.apple.com) :
...
- Dans _TYPE D'ENREGISTREMENT_, sélectionnez **CD_Book**, car nous voulons regarder nos livres
...
Retournez à Données > Enregistrements, définissez vos filtres et commutateurs : Base de données privée, zone `com.apple.coredata.cloudkit.zone`, TYPE D'ENREGISTREMENT à CD_Books et cliquez sur **Query Records** et Baammm :

![texte alternatif](query_records.png)

### Synchronisation en temps réel

L'une des fonctionnalités très cool de CloudKit est la synchronisation quasi en temps réel : Vous modifiez des données sur un appareil, et les modifications deviennent visibles sur l'autre appareil très rapidement - bien sûr sur des appareils connectés avec le même AppleId.

La façon dont cela fonctionne est qu'une notification est envoyée à tous les appareils connectés avec cet AppleId, puis ils commencent une synchronisation.

Malheureusement, les simulateurs ne reçoivent pas ces notifications. Donc, pour le tester, vous devez apporter des modifications sur un simulateur et les voir apparaître sur votre véritable appareil. Bien sûr, cela fonctionne également si vous utilisez 2 appareils physiques.
...
- [Activer CloudKit dans votre application](https://developer.apple.com/documentation/cloudkit/enabling-cloudkit-in-your-app) : Explique comment configurer votre application pour stocker des données dans iCloud à l'aide de CloudKit.
...
### Les droits ont été modifiés pendant la compilation
...
- Dans Xcode, allez à _Project > Target > Signing & Capabilities_
