...
excerpt: Un guide étape par étape pour ajouter la synchronisation iCloud à une application SwiftData en utilisant CloudKit, y compris la configuration, le débogage et les pièges courants.
...
translate_date: 2026-02-27T10:18:19.394346+00:00
generated_by: simplified-translation-system
---

**Remarque** : Le dépôt GitHub associé à cet article est [ici](https://github.com/tillg/SwiftDataOniCloud).

En apprenant Swift & SwiftUI et en commençant à l'utiliser, j'ai souvent eu du mal à utiliser CloudKit. Trop souvent... Voici une configuration minimale que j'ai utilisée pour me rappeler comment j'ai configuré les choses et documenter mes apprentissages.
...
- [Enquêter, tester, expérimenter, peut-être comprendre...](#enquêter-tester-expérimenter-peut-être-comprendre)
...
- [Erreurs fréquentes](#erreurs-fréquentes)
  ...
  - [Impossible de s'initialiser sans un compte iCloud (CKAccountStatusNoAccount).](#impossible-de-sinitialiser-sans-un-compte-icloud-ckaccountstatusnoaccount)

## Point de départ

Presque tout ce que je sais sur SwiftUI vient du superbe tutoriel [100 Days of SwiftUI](https://www.hackingwithswift.com/100/swiftui) de Paul Hudson. Il fait un travail fantastique en expliquant SwiftUI et offre ce cours bien entretenu gratuitement.

Pour ne pas avoir à tout recommencer de zéro, j'utilise la leçon du cours qui introduit SwiftData : Au [Jour 53](https://www.hackingwithswift.com/100/swiftui/53), Paul commence un nouveau projet appelé _Bookworm_ qui utilise SwiftData pour stocker ses données de livres. À la fin du [Jour 55](https://www.hackingwithswift.com/100/swiftui/55), il a construit une petite application sympathique qui stocke ses données dans SwiftData. Si vous n'êtes pas à l'aise avec SwiftData ou si vous ne vous en souvenez pas, je vous recommande fortement de regarder les vidéos de ces 3 jours. Son application utilise SwiftData, mais pas dans le cloud, juste localement sur l'appareil.
...
Si vous ouvrez `Xcode > Project: MiniSwiftData > Target: MiniSwiftData > Signing & Capabilities`, voici ce que vous devriez voir :
...
Puis dans la section `iCloud`, sélectionnez CloudKit comme Service :

![texte alternatif](cloudkit_service.png)

Ensuite, en dessous, nous avons besoin d'un conteneur. Cliquez sur `+` et entrez un nom. Le nom habituel serait l'identifiant de bundle, dans mon cas, c'est `com.grtnr.Bookworm` (je le copie-colle simplement depuis le champ _Bundle Identifier_).
...
C'est rouge parce qu'il n'a pas encore été créé, cela prend simplement un peu de temps.
...
Puis dans la liste des services de la section _Background Modes_, sélectionnez _Remote notifications_ :

![texte alternatif](remote_notifications.png)

**Remarque** : Probablement à ce stade, le nom du conteneur est passé du rouge au noir car il a été créé dans iCloud.
...
Maintenant, je lance l'application sur un simulateur, j'entre 3 livres, je la lance sur mon véritable iPhone - et voilà, les livres apparaissent sur mon iPhone !! 🥰

## Enquêter, tester, expérimenter, peut-être comprendre...

### Console CloudKit

Lors de l'enquête sur ce qui se passe, il y a un outil très utile : La [Console CloudKit](https://icloud.developer.apple.com) :
...
- Dans le menu déroulant de la zone, sélectionnez celle qui n'est pas \__default_, dans mon cas, c'est **com.apple.coredata.cloudkit.zone**. Je n'ai aucune idée (encore) de ce dont il s'agit...
- Dans _RECORD TYPE_, sélectionnez **CD_Book**, car nous voulons examiner nos livres
...
Retournez à Données > Enregistrements, définissez vos filtres et commutateurs : Base de données privée, zone `com.apple.coredata.cloudkit.zone`, TYPE D'ENREGISTREMENT sur CD_Books et cliquez sur **Query Records** et Bammm :
...
Malheureusement, les simulateurs ne reçoivent pas ces notifications. Donc, pour le tester, vous devez apporter des modifications sur un simulateur et les voir apparaître sur votre appareil réel. Bien sûr, cela fonctionne également si vous utilisez 2 appareils physiques.
...
- [Activation de CloudKit dans votre application](https://developer.apple.com/documentation/cloudkit/enabling-cloudkit-in-your-app) : Explique comment configurer votre application pour stocker des données dans iCloud en utilisant CloudKit.
- [Gestion des conteneurs iCloud avec l'application CloudKit Database](https://developer.apple.com/documentation/cloudkit/managing-icloud-containers-with-cloudkit-database-app#//apple_ref/doc/uid/TP40014987-CH5) : Explique comment enquêter et voir les données dans vos conteneurs via le Web Dev d'Apple.

- [TN3164 : Débogage de la synchronisation de NSPersistentCloudKitContainer](https://developer.apple.com/documentation/technotes/tn3164-debugging-the-synchronization-of-nspersistentcloudkitcontainer) : En essayant de comprendre pourquoi mon application ne pouvait pas se connecter à son conteneur et continuait de signaler des problèmes d'accès, voici la phrase clé de cette note technique qui m'a sauvé :

> Si le portail montre que l'association entre votre conteneur CloudKit et l'ID d'application est correcte, mais que l'erreur persiste, c'est probablement parce que l'association n'est pas synchronisée avec le serveur CloudKit. Dans ce cas, envisagez d'utiliser un nouveau conteneur CloudKit pour continuer votre développement.
...
Bien sûr, je n'ai pas modifié les droits pendant que Xcode compilait. Mais la façon de résoudre ce problème est assez simple :

- Dans Xcode, allez dans _Project > Target > Signing & Capabilities_
- Décochez (désélectionnez) _Automatically manage signing_
- Re-sélectionnez-le
- Sélectionnez votre _Team_

...c'est tout : Xcode reconstruit maintenant votre fichier de droits.

### Impossible de s'initialiser sans un compte iCloud (CKAccountStatusNoAccount).
...
_Paramètres > iCloud > Enregistré sur iCloud | Voir tout_. C'est à quoi ressemblait le coupable :
