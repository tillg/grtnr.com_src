...
translate_date: 2026-02-05T20:59:47.736817
generated_by: simplified-translation-system
---

**Remarque** : Le dépôt GitHub associé à cet article est [ici](https://github.com/tillg/SwiftDataOniCloud).

Alors que j'apprenais Swift & SwiftUI et commençais à l'utiliser, j'ai souvent eu des difficultés à utiliser CloudKit. Trop souvent... Voici une configuration minimale que j'ai utilisée pour me rappeler à l'avenir comment j'ai configuré les choses et documenter mes apprentissages.
...
Presque tout ce que je sais sur SwiftUI provient de l'excellent tutoriel [100 Days of SwiftUI](https://www.hackingwithswift.com/100/swiftui) de Paul Hudson. Il fait un travail fantastique pour expliquer SwiftUI et offre ce cours bien entretenu gratuitement.

Pour ne pas avoir à tout recommencer de zéro, j'utilise la leçon du cours qui introduit SwiftData : Au [jour 53](https://www.hackingwithswift.com/100/swiftui/53), Paul commence un nouveau projet appelé _Bookworm_ qui utilise SwiftData pour stocker ses données de livres. À la fin du [jour 55](https://www.hackingwithswift.com/100/swiftui/55), il a construit une petite application mignonne qui stocke ses données dans SwiftData. Si vous n'êtes pas à l'aise avec SwiftData ou ne vous en souvenez pas, je vous recommande fortement de regarder les vidéos de ces 3 jours. Son application utilise SwiftData, mais pas dans le cloud, juste localement sur l'appareil.

Je vais utiliser cela comme point de départ pour y ajouter la synchronisation iCloud. Vous pouvez trouver l'application _Bookworm_ dans le [projet Github de Paul Hudson](https://github.com/twostraws/HackingWithSwift) : Elle est sous [SwiftUI > project 11](https://github.com/twostraws/HackingWithSwift/tree/main/SwiftUI/project11).
...
C'est en rouge car il n'a pas encore été créé, cela prend simplement un peu de temps.

Pour garder les appareils synchronisés, iCloud utilise les _Notifications Push à Distance_, donc nous devons également ajouter ce service. Cliquez à nouveau sur `+ Capability` et recherchez _Back..._ :
...
Lors du démarrage de l'application, nous obtenons une erreur similaire à celle-ci :
...
- Dans le menu déroulant de la zone, sélectionnez celle qui n'est pas \__default_, dans mon cas, c'est **com.apple.coredata.cloudkit.zone**. Je n'ai aucune idée (pour l'instant) de ce que sont ces zones...
- Dans _TYPE D'ENREGISTREMENT_, sélectionnez **CD_Book**, car nous voulons examiner nos livres

Cliquez sur _Interroger les enregistrements_ et obtenez une erreur... 😂
...
Nous voyons qu'il avait raison avec son message d'erreur : `recordName` n'est pas marqué comme interrogeable. Réparons cela.
...
Retournez à Données > Enregistrements, définissez vos filtres et commutateurs : Base de données privée, zone `com.apple.coredata.cloudkit.zone`, TYPE D'ENREGISTREMENT sur CD_Books et cliquez sur **Interroger les enregistrements** et Baammm :
...
- [Gérer les conteneurs iCloud avec l'application CloudKit Database](https://developer.apple.com/documentation/cloudkit/managing-icloud-containers-with-cloudkit-database-app#//apple_ref/doc/uid/TP40014987-CH5) : Explique comment investiguer et voir les données dans vos conteneurs via le Web Dev d'Apple.
...
Les champs d'un modèle doivent avoir des valeurs par défaut pour pouvoir être enregistrés dans CloudKit. Vous n'en avez pas besoin pour SwiftData si vous enregistrez simplement les données localement sur l'appareil.
...
Bien sûr, je n'ai pas modifié les droits pendant que Xcode construisait. Mais la solution est assez simple :
