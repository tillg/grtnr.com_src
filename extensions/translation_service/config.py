...
class TranslationConfig:
    ...
    def from_dotenv(cls, dotenv_path: str = None) -> 'TranslationConfig':
        ...
        try:
            ...
            raise ConfigurationError("python-dotenv package is required for .env file support")
    ...
    def from_environment(cls) -> 'TranslationConfig':
        ...
        config.max_retries = int(os.getenv("TRANSLATION_MAX_RETRIES", str(config.max_retries)))
        ...
        config.auto_detect_language = os.getenv("TRANSLATION_AUTO_DETECT", "true").lower() == "true"
    ...
    def from_pelican_settings(cls, settings: dict) -> 'TranslationConfig':
        ...
        # API Configuration
        config.api_key = settings.get("TRANSLATION_API_KEY", os.getenv("OPENAI_API_KEY", ""))
        if not config.api_key:
            raise ConfigurationError("Translation API key not found in settings or environment")
        
        config.model = settings.get("TRANSLATION_MODEL", config.model)
        
        # Language Configuration
        config.target_languages = settings.get("TRANSLATION_TARGET_LANGUAGES", config.target_languages)
        config.exclude_categories = settings.get("TRANSLATION_EXCLUDE_CATEGORIES", config.exclude_categories)
        config.exclude_paths = settings.get("TRANSLATION_EXCLUDE_PATHS", config.exclude_paths)
        
        # Cache Configuration
        config.cache_enabled = settings.get("TRANSLATION_CACHE_ENABLED", config.cache_enabled)
        ...
        config.rate_limit_delay = settings.get("TRANSLATION_RATE_LIMIT_DELAY", config.rate_limit_delay)
        
        # Language Detection
        config.auto_detect_language = settings.get("TRANSLATION_AUTO_DETECT", config.auto_detect_language)
        config.default_source_language = settings.get("TRANSLATION_DEFAULT_SOURCE_LANG", config.default_source_language)
    ...
    def validate(self) -> None:
        ...
        valid_languages = {
            "en", "de", "fr", "es", "it", "pt", "ru", "ja", "ko", "zh", "ar", "hi", "nl", "sv", "no", "da", "fi"
        }
