...
LANGUAGE CODE:"""
    ...
    @classmethod
    def build_translation_prompt(cls, content: str, source_lang: str, target_lang: str) -> str:
        ...
        if target_lang.lower() in cls.LANGUAGE_ADAPTATIONS:
            ...
            if 'technical_terms' in adaptations:
                additional_notes.append(f"TECHNICAL TERMS: {adaptations['technical_terms']}")
            ...
            if additional_notes:
                ...
                if insertion_point != -1:
                    ...
                    prompt = prompt[:insertion_point] + notes_text + prompt[insertion_point:]
    ...
    @classmethod
    def validate_translation_response(cls, response: str, original_content: str) -> Dict[str, bool]:
        """Validate translation response for common issues"""
        
        validation_results = {
            'has_content': len(response.strip()) > 0,
            'preserves_markdown_headers': original_content.count('#') == response.count('#'),
            'preserves_code_blocks': original_content.count('```') == response.count('```'),
    ...
    @classmethod
    def suggest_prompt_improvements(cls, validation_results: Dict[str, bool]) -> List[str]:
        ...
        if not validation_results.get('reasonable_length', True):
            suggestions.append("Review translation length - may be too short or too long")
