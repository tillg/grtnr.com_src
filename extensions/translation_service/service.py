...
class TranslationAPIClient:
    ...
    def translate(
        ...
        prompt = self.prompts.build_translation_prompt(
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                validation_results = self.prompts.validate_translation_response(
                    translation, content
                )

                if not validation_results.get("has_content", True):
                    raise InvalidResponseError(
                        "Translation validation failed: no content"
                    )

                # Extract model information from response
                model_info = (
                    response.model if hasattr(response, "model") else self.config.model
                )
        ...
        if "rate_limit" in str(last_exception).lower():
            ...
            raise APIError(
                f"API call failed after {self.config.max_retries} retries: {last_exception}"
            )
    ...
    def detect_language(self, content: str) -> str:
        ...
        try:
            ...
            if self.prompts.is_language_supported(detected_lang):
                ...
                self.logger.warning(
                    f"Detected unsupported language '{detected_lang}', using default"
                )
                return self.config.default_source_language
            ...
            if len(lines) > 2:
                ...
                for i in range(len(lines) - 1, 0, -1):  # Search backwards
                    if lines[i].strip() == "```":
                        ...
                        self.logger.debug(
                            "Removed ```markdown wrapper from translation"
                        )
                        break
...
class BatchTranslationProcessor:
    ...
    def process_batch(
        self, contents: List[str], source_lang: str, target_lang: str, translation_func
...
class TranslationService:
    ...
    def translate_content(
        ...
        try:
            translation, model_info = self.api_client.translate(
                content, source_lang, target_lang
            )
    ...
    def translate_batch(
        self, contents: List[str], source_lang: str, target_lang: str
    ) -> List[TranslationResult]:
        """Translate multiple contents in batch"""
        return self.batch_processor.process_batch(
            contents, source_lang, target_lang, self.translate_content
        )
